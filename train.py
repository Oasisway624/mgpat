...
class Net:
    def __init__(self, args):
        ...
        ##########################################################################################################################################
        if self.model_name == 'MPGAT':
            self.features = [self.v_feat, self.a_feat, self.t_feat]
            self.model = MPGAT(self.features, self.edge_index, self.batch_size, self.num_user, self.num_item,
                               self.dim_latent).cuda()
        elif self.model_name == 'MGAT':
            self.features = [self.v_feat, self.a_feat, self.t_feat]
            self.model = MPGAT(self.features, self.edge_index, self.batch_size, self.num_user, self.num_item,
                               self.dim_latent).cuda()
        ...
        elif self.model_name == 'CL4SREC':
            self.dim_feat = self.dim_v + self.dim_a + self.dim_t
            self.features = torch.tensor(np.concatenate((self.v_feat, self.a_feat, self.t_feat), axis=1),
                                         dtype=torch.float)
            # self.model = CL4SREC(self.features, self.edge_index, self.batch_size, self.num_user, self.num_item, self.dim_latent, self.dim_feat).cuda()
        elif self.model_name == 'SASREC':
            self.dim_feat = self.dim_v + self.dim_a + self.dim_t
            self.features = torch.tensor(np.concatenate((self.v_feat, self.a_feat, self.t_feat), axis=1),
                                         dtype=torch.float)
            # self.model = SASREC(self.features, self.edge_index, self.batch_size, self.num_user, self.num_item, self.dim_latent, self.dim_feat).cuda()
        elif self.model_name == 'ACF':
            self.dim_feat = self.dim_v + self.dim_a + self.dim_t
            self.features = torch.tensor(np.concatenate((self.v_feat, self.a_feat, self.t_feat), axis=1),
                                         dtype=torch.float)
            # self.model = ACF(self.features, self.edge_index, self.batch_size, self.num_user, self.num_item, self.dim_latent, self.dim_feat).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='MPGAT', help='Model name.')
